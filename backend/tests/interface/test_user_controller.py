...
class TestGetUsers:
    ...
    def test_get_users_when_normal_user(self, client, token):
        ...
        assert response.json() == {'detail': 'Only admin users can retrieve the user list'}
...
class TestUpdateUser:
    ...
    async def test_update_user_when_itself(self, client, user_repository, token):
        ...
        response = client.put(
            f'/users/{user.id}',
            json={'username': 'anotherusername', 'first_name': 'Another', 'last_name': 'User'},
            headers={'Authorization': f'Bearer {token}'},
        )
        assert response.status_code == status.HTTP_200_OK
        assert response.json() == {
    ...
    def test_update_user_when_not_permission(self, client, token):
        response = client.put(
            '/users/999',
            json={'username': 'anotherusername', 'first_name': 'Another', 'last_name': 'User'},
            headers={'Authorization': f'Bearer {token}'},
    ...
    async def test_update_user_when_username_already_exists(self, client, token, user_repository):
        ...
        response = client.put(
            f'/users/{user.id}',
            json={'username': 'testuser2', 'first_name': 'Another', 'last_name': 'User'},
            headers={'Authorization': f'Bearer {token}'},
        )
